...
from app.services.vector_db import vector_db # IMPORT PINECONE
from app.services.email_sender import email_sender # IMPORT EMAIL SENDER SERVICE
...
def generate_email_line(prospect_id: int, db: Session = Depends(get_db)):
    ...
    personalized_line = llm.generate_opening_line(
        ...
        company_name=prospect.company_name,
        scraped_context=retrieved_context or scraped_data # Fallback to raw data if search fails
    )
    ...
    return {
        ...
        "rag_context_used": retrieved_context[:200] + "...", # Show a snippet of what Pinecone found
